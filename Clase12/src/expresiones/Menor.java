...
public class Menor extends Instruccion {
    ...
    public Object interpretar(Arbol arbol, tablaSimbolos tabla) {
        ...
        return switch (comparando1) {
            case ENTERO -> switch (comparando2) {
                case ENTERO -> (int) cond1 < (int) cond2;
                case DECIMAL -> (int) cond1 < (double) cond2;
                default -> new Errores("SEMANTICO",
                        "Relacional Invalido",
                        this.linea,
                        this.col);
            };
            case DECIMAL -> switch (comparando2) {
                case ENTERO -> (double) cond1 < (int) cond2;
                case DECIMAL -> (double) cond1 < (double) cond2;
                default -> new Errores("SEMANTICO",
                        "Relacional Invalido",
                        this.linea,
                        this.col);
